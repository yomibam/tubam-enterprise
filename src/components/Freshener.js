...
export default function Freshener() {
  const HandleFreshenerClick = (freshenerInfo) => {
    return (e) => {
      e.preventDefault();
      window.open(
        `https://wa.me/2348035647633?text=Hello,%20I'd%20like%20to%20place%20an%20order%20for%20${freshenerInfo.name},%20${freshenerInfo.volume}.`,
        "_blank",
        "noopener",
        "noreferrer"
      );
    ...
    return airFreshenerData.map((freshener) => (
      ...
      >
        <div className="w-[150px] h-[150px] overflow-hidden bg-white lap:w-[300px] lap:h-[300px]">
          <img
            src={require(`../assets/${freshener.img_url}`)}
            alt={freshener.name}
  ...
  return (
    <div className="flex items-center flex-col py-[20px] px-[10px] gap-[10px] lap:mt-24 lap:gap-8">
      <h2 className="text-base font-bold lap:text-xxl">Air Freshener</h2>
      <div
